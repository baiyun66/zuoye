...
public class QuadTree {
    ...
    public Node find(Node node, double x, double y) {
        ...
        else throw new QuadTreeException("Invalid nodeType");
        return resposne;
    }
    ...
    private boolean insert(Node parent, Point point) {
        ...
        if(parent.getNodeType()==NodeType.EMPTY) {
                this.setPointForNode(parent, point);
                result = true;
        }
        else if(parent.getNodeType()==NodeType.LEAF){
        ...
        else if(parent.getNodeType()==NodeType.POINTER){
                result = this.insert(
                        this.getQuadrantForPoint(parent, point.getX(), point.getY()), point);
        }
	    else	throw new QuadTreeException("Invalid nodeType in parent");
        return result;
    }
    ...
    public void balance(Node node) {
    	if(node.getNodeType()==NodeType.EMPTY){}
                ...
                else if (ne.getNodeType() != NodeType.EMPTY) {
                    firstLeaf = ne;
                }
                else if (sw.getNodeType() != NodeType.EMPTY) {
                firstLeaf = sw;
                }
                else if (se.getNodeType() != NodeType.EMPTY) {
                firstLeaf = se;
                }
                if (firstLeaf == null) {
                ...
                if (firstLeaf.getNodeType() != NodeType.POINTER&&node.getParent() != null) {
                    this.balance(node.getParent());
                }
    	}
    }
